...
VERSION: 1.0
DESCRIPTION: Assignment 2, in CSE 415, Autumn 2019.
...
print("\nWelcome to UCS, by Nitya Krishna Kumar!")
...
# STEP 2. If OPEN is empty, output “DONE” and stop.
  # False meaning OPEN is not empty
  while (len(OPEN) != 0): 
    if VERBOSE: 
   ...
   # EACH SUCCESSOR.***
    ...
    for operator in Problem.OPERATORS: 
        if operator.precond(S):
           ...
           total_cost = P + S.edge_distance(news)
           if not (news in OPEN or news in CLOSED):
               OPEN.insert(news, total_cost)
               BACKLINKS[news] = S
           else:
               if (total_cost < OPEN[news]):
                   del OPEN[news]
                   OPEN.insert(news, total_cost)
                   BACKLINKS[news] = S
               del news  
